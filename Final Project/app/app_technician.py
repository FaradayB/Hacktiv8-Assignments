...
try:
    from db import get_track1_history, get_query_log
    DB_AVAILABLE = True
except Exception:
    DB_AVAILABLE = False
...
def run_chat_query(question: str, result: dict) -> str:
    ...
    try:
        ...
        system = f"""You are an expert automotive diagnostic assistant helping a Mitsubishi workshop technician.
        ...
        for msg in st.session_state.chat_history:
            ...
            if msg["role"] == "user":
                ...
                chat_html += (
                    ...
                    f'<div><div class="chat-label">MitsuCare Assistant</div>'
                    f'<div class="bubble">{content_safe}</div></div>'
                    f'</div>'
                )
        chat_html += '</div>'
